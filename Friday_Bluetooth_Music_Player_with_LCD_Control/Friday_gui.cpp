...
Friday_gui::Friday_gui(QWidget *parent) : QWidget(parent)
{
	...
	*songList = dir.entryList();
	...
	layout->addWidget(cpeople, 1, 2);
	layout->addWidget(reset, 2, 2);
	layout->addWidget(cmode, 3, 2);	
	layout->addWidget(change, 4, 2);
...
void Friday_gui::next_handler()
{
	...
	if(state == 1){
		stop_handler();
		std::string file = "ash /media/card/project/scripts/music_control.sh -s /media/card/project/mp3s/" + (*songList)[csong];
		system(file.c_str());
	}
}
